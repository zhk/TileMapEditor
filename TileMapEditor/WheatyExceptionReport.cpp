...
PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    sprintf(m_szLogFileName, "%s\\%s_[%4u-%02u-%02u_%02u-%02u-%02u].txt",
	...
	{
		char szMDFile[MAX_PATH];
		strcpy( szMDFile, m_szLogFileName );
		//strcat(szMDFile, ".mdmp");
		sprintf( strrchr( szMDFile, '.' ) + 1, "mdmp" );

		// If file is exist, delete it;
		if ( SetFileAttributesA( szMDFile, FILE_ATTRIBUTE_NORMAL ) )
			DeleteFileA( szMDFile );

		HANDLE hFile = CreateFileA( szMDFile, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
...
PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	_tprintf(_T("Date %4u-%02u-%02u Time %02u:%02u:%02u \r\n"), systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);
