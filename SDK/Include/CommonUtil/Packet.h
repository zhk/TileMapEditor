...
namespace Cactus
{
	...
	*/
	...
	public:
		...
		template<> bool Peek(Cactus::String& str, size_t pos);
		template<> bool Peek(Cactus::WString& str, size_t pos);
		template<> bool Peek(Packet& p, size_t pos);
		template<> bool Peek(NatureTime& t, size_t pos);
		...
		template<> size_t Patch(const Cactus::String& str, size_t pos);
		template<> size_t Patch(const Cactus::WString& str, size_t pos);
		template<> size_t Patch(const Packet& p, size_t pos);
		template<> size_t Patch(const NatureTime& t, size_t pos);
	...
	inline size_t Patch(Cactus::Packet& os, const BitSet<bits>& bt, size_t pos)
	{
		...
		uint16 lenOld;
		if( Peek(lenOld, pos) && sz == lenOld )
		{
			pos += sizeof(uint16);

			for (uint16 t = 0; t < sz; ++t)
			{
				if( !Patch(bt.getword(t), pos) )
				{
					return 0;
				}
